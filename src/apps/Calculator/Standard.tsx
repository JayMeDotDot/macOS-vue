...
export default defineComponent({
  ...
  setup() {
    ...
    function standardCal(event: MouseEvent) {
      ...
      if (key.value === '+/-') {
        ...
        num >= 0 ? (initVal.value = '-' + num) : (initVal.value = '' + -num)
        initFlag.value = false
        return
      }
      if (key.value === '.') {
  ...
  render() {
    const { ACOrC, initVal, standardCal } = this
        ...
        <div class="calculator-keys" onClick={standardCal}>
          {CalUI.standard.map(item => {
            if (item === 0) {
              return (
                <button
                  class="calculator-key dark:calculator-key-dark active:calculator-key-active active:dark:calculator-key-dark-active w-auto col-span-2"
                  value={item}
                >
                  {item}
                </button>
            ...
            if (item === 'AC') {
              return (
                <button
                  class="calculator-key dark:calculator-key-dark active:calculator-key-active active:dark:calculator-key-dark-active "
                  value={ACOrC}
                >
                  {ACOrC}
                </button>
